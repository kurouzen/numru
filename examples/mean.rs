...
fn main() {
    ...
    let c_mean = c.mean().compute().unwrap();
    println!("c.mean() = {:?}", c_mean);
    let c_mean_axis_0 = c.mean().axis(0).compute().unwrap();
    println!("c.mean().axis(0) = {:?}", c_mean_axis_0);
    let c_mean_axis_1 = c.mean().axis(1).compute().unwrap();
    println!("c.mean().axis(1) = {:?}", c_mean_axis_1);
    let c_mean_axis_2 = c.mean().axis(2).compute().unwrap();
