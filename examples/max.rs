...
fn main() {
    ...
    let c_max = c.max().compute().unwrap();
    println!("c.max() = {:?}", c_max);
    let c_max = c.max().axis(0).compute().unwrap();
    println!("c.max().axis(0) = {:?}", c_max);
    let c_max = c.max().axis(1).compute().unwrap();
    println!("c.max().axis(1) = {:?}", c_max);
    let c_max = c.max().axis(2).compute().unwrap();
